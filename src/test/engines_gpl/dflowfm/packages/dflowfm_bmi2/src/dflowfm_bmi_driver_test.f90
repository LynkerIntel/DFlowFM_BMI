...
program dflowfm_driver_test
  ...
  !---------------------------------------------------------------------
    ...
    integer                                           :: grid_size, grid1_size, grid2_size, grid3_size, grid4_size, grid5_size, grid6_size, grid7_size        ! size of grid (ie. nX * nY)
    ...
    logical :: success_test, available
    integer :: iitem, loop
  ...
  ! which here highlight the node and element coordinates of the mesh
  status = m%get_grid_size(1, grid1_size)
  status = m%get_grid_size(2, grid2_size)
  
  ! Check to see if inland discharge boundaries are allocated for DFlowFM setup
  if(nbndu > 0) then
  ...
  ! Check to see if precipitation forcings are allocated for DFlowFM setup
  ...
  ! Check to see if wind forcings are allocated for DFlowFM setup
  if(allocated(wx) .and. allocated(wy)) then  
  ...
  ! Check to see if surface pressure forcings are allocated for DFlowFM setup
  ...
  ! Check to see if lateral discharge forcings (possibly discharge and lateral discharge combined) are allocated for DFlowFM setup
  if(allocated(qplat)) then

      status = m%get_grid_size(7, grid7_size)

      allocate(LATQ_t0(grid7_size))
      allocate(LATQ_t1(grid7_size))

      LATQ_t0(:) = 10.0
      status = m%set_value('LATQ_t0', LATQ_t0)
      LATQ_t1(:) = 15.0
      status = m%set_value('LATQ_t1', LATQ_t1)

      print*, 'LATQ_t0 new data', qplat_t0(1)
      print*, 'LATQ_t1 new data', qplat_t1(1)
  ...
  !---------------------------------------------------------------------
    ...
    ! Initalize the boolean flag before loop for 
    ! checking grid availability based on DFlowFM setup
    available = .False.

    ! Loop over grids and get information
    do grid_int=1, 7
      if(grid_int .eq. 3 .and. nbndu > 0) then
          available = .True.
      elseif(grid_int .eq. 4 .and. nbndu > 0) then
          available = .True.
      elseif(grid_int .eq. 5 .and. nbndz > 0) then
          available = .True.
      elseif(grid_int .eq. 6 .and. nbndz > 0) then
          available = .True.
      elseif(grid_int .eq. 7 .and. allocated(qplat)) then
          available = .True.
      elseif(grid_int .eq. 1 .or. grid_int .eq. 2) then
          available = .True.
      endif
